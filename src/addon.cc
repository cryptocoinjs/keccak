...
class KeccakWrapper : public Nan::ObjectWrap {
 ...
 private:
  ...
  static NAN_METHOD(Initialize) {
    ...
    // ignore return code, rate & capacity always will right because internal
    // object
  ...
  static NAN_METHOD(Absorb) {
    ...
    const unsigned char* data =
        (const unsigned char*)node::Buffer::Data(buffer);
...
#if (NODE_MODULE_VERSION >= NODE_7_0_MODULE_VERSION)
    KeccakWrapper* to = Nan::ObjectWrap::Unwrap<KeccakWrapper>(
        info[0]
            ->ToObject(info.GetIsolate()->GetCurrentContext())
            .ToLocalChecked());
#else
    KeccakWrapper* to =
        Nan::ObjectWrap::Unwrap<KeccakWrapper>(info[0]->ToObject());
#endif
